...
- **Cascade policy**: deleting a document cascades to its extractions
  and those cascade to critic_reviews (an orphaned extraction is
  meaningless); signals and backtest_runs never cascade from upstream
  deletes since they're downstream aggregates that should persist.
- **Conflict resolution** (signal/aggregator.py): critic-rejected
  extractions are excluded entirely, not down-weighted — rejected
  evidence isn't partial evidence, it's noise. Surviving extractions are
  weighted by `confidence × source_type_weight` (filings > transcripts >
  news).
- **Idempotency via constraint, not application logic**: re-running
  ingestion is safe because `content_hash UNIQUE` makes duplicate
  inserts a no-op — `DocumentRepository.insert_if_new` just returns
  `None`, no "have I seen this?" check needed upstream.
- **Views vs. Python**: the dashboard is intentionally dumb — every
  panel is `pd.read_sql` against a view. Aggregation logic lives in one
  place (SQL), not duplicated between backend and dashboard code.

## Limitations & Next Steps

- **No live LLM run performed in this environment.** The extractor/critic
  agents are real, complete code but haven't been executed against a
  live API key + live Postgres here — do that locally before relying on
  any numbers in this README.
- **Transcript source is a small hand-collected sample** (see
  `ingestion/transcript_source.py` docstring), not a live feed — don't
  represent it as one.
- **EDGAR full-text search endpoint is undocumented/unofficial** and its
  response shape can change; treat `ingestion/edgar_source.py` as a
  starting point to verify against current EDGAR behavior, not a stable
  contract.
- **Golden set is 5 illustrative rows**, not the real 30-50 the spec
  calls for — `config/golden_set.csv` needs a real manual labeling pass.
- **Backtest uses next-day forward returns you must supply** (see
  `run_backtest`'s `returns_by_ticker_date` param) — no price-data
  ingestion is wired up yet; the cleanest next step is a `PriceSource`
  following the same `DataSource`-style interface, landing daily closes
  in a new `prices` table.
- **No CI pipeline** wired up yet (GitHub Actions running `pytest` +
  `alembic upgrade head` against a service-container Postgres would be
  the natural next step).
- **Materialized views**: the three views in `db/views.sql` are plain
  views (always current, recomputed per query). If the dashboard needs
  to scale past a small backtest history, converting
  `v_signal_vs_baseline_performance` to a materialized view with a
  scheduled `REFRESH MATERIALIZED VIEW` would be the next optimization.
